...
function writeReview() {
  ...
  if (
    localStorage.getItem(
      document.getElementsByClassName("movieId")[0].id + "/" + reviewer
    ) !== null
  ) {
  ...
  if (
    localStorage.getItem(document.getElementsByClassName("movieId")[0].id) ===
    null
  ) {
    const arr = [reviewer];
    localStorage.setItem(
      document.getElementsByClassName("movieId")[0].id,
      JSON.stringify(arr)
    );
  } else {
...
function deleteBtn() {
  ...
  const reviewer = this.parentNode
    .querySelector(".reviewer")
    .textContent.trim(); // 리뷰어 이름 가져오기
  const localPW = JSON.parse(
    localStorage.getItem(
      document.getElementsByClassName("movieId")[0].id + "/" + reviewer
    )
  )[1]; // 해당 리뷰어의 비밀번호 가져오기
  ...
  if (promptPW === localPW) {
    ...
    localStorage.setItem(
      document.getElementsByClassName("movieId")[0].id,
      JSON.stringify(updateReviewers)
    );
...
function updateBtn() {
  ...
  if (promptPW === localPW) {
    ...
    document.getElementById("updateReviewer").textContent = this.parentNode
      .querySelector(".reviewer")
      .textContent.trim();
  } else if (promptPW == null) {
    //프롬프트 취소했을 경우
  ...
  if (
    ...
    for (let i = reviewers.length - 1; i >= 0; i--) {
      const reviewData = JSON.parse(
        localStorage.getItem(
          document.getElementsByClassName("movieId")[0].id + "/" + reviewers[i]
        )
      );
